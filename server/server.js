...
app.post('/api/register', async (req, res) => {
    ...
    try {
        ...
        const mailOptions = {
            ...
            text: `Привіт ${username}! Ви успішно реєструвалися на нашому сайті. Ваш логін ${email}. Ваш пароль ${password}`
        };

        await transporter.sendMail(mailOptions);

        res.status(201).json({ message: 'Користувач успішно зареєстрований, повідомлення надіслано на пошту' });
    } catch (error) {
        console.error('Ошибка при регистрации пользователя:', error);
        res.status(500).json({ error: 'Помилка під час реєстрації користувача' });
    }
});
...
app.post('/api/registerorder', async (req, res) => {
    ...
    try {
        ...
        if (bookedSlots.length > 0 || unBookedSlots.length > 0) {
            ...
            return res.status(status).json({
                message: bookedSlots.length > 0
                    ? 'Деякі слоти вже заброньовані користувачем.'
        ...
        console.error('Помилка під час реєстрації замовлення:', error);
...
app.put('/api/updatePaymentStatus', async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ message: 'Статус платежу успішно оновлено', order });
    } catch (error) {
...
app.get('/api/checkpaymentstatus', async (req, res) => {
    ...
    try {
        ...
        console.error('Помилка під час перевірки статусу оплати:', error);
        res.status(500).send({ message: 'Помилка під час перевірки оплати' });
    }
});
...
app.post('/api/liqpay-webhook', (req, res) => {
    ...
    if (paymentData.status === 'success') {
        ...
        console.log(`Платіж ${paymentData.order_id} успішно завершено`);
